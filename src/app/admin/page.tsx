...
interface Question {
  ...
  topic: string; year: number | null; is_active: boolean; has_math: boolean
  subjects?: { name: string; code: string }
}
interface Profile {
  id: string; full_name: string; email: string; target_exam: string
  total_tests_taken: number; total_score_points: number; created_at: string
}
...
interface ExtractedQ {
  ...
  correct_option: string; explanation: string; subject: string; topic: string
  difficulty: string; year: number | null; has_math: boolean
}
interface QForm {
  exam_type: string; subject_id: string; question_text: string
  option_a: string; option_b: string; option_c: string; option_d: string
  correct_option: string; explanation: string; difficulty: string
  topic: string; year: string; has_math: boolean
}
...
const EMPTY_Q: QForm = {
  ...
  topic: '', year: '', has_math: false,
}

const ALL_EXAMS = ['JEE', 'VITEEE', 'KEAM', 'CUSAT', 'NEET']
    ...
    <span>
      {parts.map((p, i) =>
        p.startsWith('$') && p.endsWith('$')
          ? <code key={i} className="px-1 py-0.5 rounded text-xs" style={{ background: 'rgba(43,175,252,0.1)', color: '#2baffc', fontFamily: 'monospace' }}>{p.slice(1, -1)}</code>
          : <span key={i}>{p}</span>
      )}
    </span>
  )
...
function AdminLogin({ onSuccess }: { onSuccess: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (res.ok) { onSuccess() } else { setError('Incorrect password'); setLoading(false) }
  }
...
function AdminPanel() {
  ...
  const [extractedQs, setExtractedQs] = useState<ExtractedQ[]>([])
  ...
  function updQ(k: keyof QForm, v: string | boolean) { setQForm(f => ({ ...f, [k]: v })) }

  function subjectsByExam(et: string) { return subjects.filter(s => s.exam_type === et) }

  async function saveQuestion() {
    setSaving(true)
    const payload = {
      exam_type: qForm.exam_type, subject_id: qForm.subject_id || null,
      question_text: qForm.question_text, option_a: qForm.option_a,
      option_b: qForm.option_b, option_c: qForm.option_c, option_d: qForm.option_d,
      correct_option: qForm.correct_option, explanation: qForm.explanation,
      difficulty: qForm.difficulty, topic: qForm.topic,
      year: qForm.year ? parseInt(qForm.year) : null, has_math: qForm.has_math,
    }
    if (editId) { await supabase.from('questions').update(payload).eq('id', editId); setEditId(null) }
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    if (file.type === 'text/plain') { setPdfText(await file.text()) }
    else { setPdfError('Upload a .txt file, or copy-paste text from PDF directly.') }
  }

  async function extractQuestions() {
    if (!pdfText.trim()) { setPdfError('Paste some text first'); return }
    ...
    try {
      ...
      setSelectedExtracted(new Set((data.questions || []).map((_: ExtractedQ, i: number) => i)))
    } catch { setPdfError('Extraction failed.') }
  ...
  async function saveExtractedQuestions() {
    ...
    setSavingPdf(true)
    for (const q of extractedQs.filter((_, i) => selectedExtracted.has(i))) {
      const subj = subjects.find(s => s.exam_type === pdfExamType && s.name.toLowerCase() === q.subject.toLowerCase())
      await supabase.from('questions').insert({
        exam_type: pdfExamType, subject_id: subj?.id || null,
        ...
        year: q.year, has_math: q.has_math, is_active: true,
      })
    }
    setSavingPdf(false); setExtractedQs([]); setPdfText(''); loadAll()
    alert(`✅ ${selectedExtracted.size} questions saved!`)
  }

  async function toggleExam(id: string, current: boolean) {
    await supabase.from('mock_exams').update({ is_active: !current }).eq('id', id); loadAll()
  }

  async function deleteExam(id: string) {
    if (!confirm('Delete exam?')) return
    await supabase.from('mock_exams').delete().eq('id', id); loadAll()
  }
  ...
  const filteredQ = questions.filter(q => {
    ...
    const matchSubj = !subjFilter || q.subjects?.name === subjFilter
    return matchText && matchSubj
  })

  const TABS = [
    { id: 'overview' as const,   label: 'Overview',                        icon: <BarChart3 size={14} /> },
    { id: 'questions' as const,  label: `Questions (${stats.totalQuestions})`, icon: <BookOpen size={14} /> },
    { id: 'pdf' as const,        label: 'PDF Import',                      icon: <FileText size={14} /> },
    { id: 'exams' as const,      label: 'Mock Exams',                      icon: <Edit3 size={14} /> },
    { id: 'students' as const,   label: `Students (${stats.totalStudents})`,   icon: <Users size={14} /> },
  ]
    ...
    <div className="min-h-screen" style={{ background: '#010101' }}>
      ...
      <header className="sticky top-0 z-30 glass" style={{ borderBottom: '1px solid #1e1e24' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <a href="/" className="text-xs" style={{ color: 'rgba(244,249,253,0.4)' }}>← Site</a>
        </div>
      </header>
        ...
        {loading ? (
          ...
          <>
              ...
              <div className="space-y-5 animate-in">
                ...
                <DashCard glowColor="blue" style={{ padding: '20px' }}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {ALL_EXAMS.map(et => (
                      <div key={et}>
                        <div className="text-xs font-mono-display font-bold mb-2 px-2 py-1 rounded w-fit"
                          style={{ background: EXAM_COLORS[et] + '15', color: EXAM_COLORS[et] }}>{et}</div>
                        <div className="flex flex-wrap gap-1.5">
                          {subjectsByExam(et).length === 0
                            ? <span className="text-xs" style={{ color: 'rgba(244,249,253,0.3)' }}>Run supabase-exams-fix.sql</span>
                            : subjectsByExam(et).map(s => (
                              <span key={s.id} className="text-xs px-2 py-1 rounded flex items-center gap-1"
                                style={{ background: (s.color || '#2baffc') + '15', color: s.color || '#2baffc', border: `1px solid ${(s.color || '#2baffc')}30` }}>
                                {s.icon} {s.name}
                              </span>
                            ))
                          }
            ...
            {tab === 'questions' && (
              <div className="space-y-4 animate-in">
                <div className="flex flex-wrap items-center gap-3">
                  <input className="input-field text-sm" style={{ maxWidth: '240px' }} placeholder="Search..." value={qFilter} onChange={e => setQFilter(e.target.value)} />
                  <select className="input-field text-sm" style={{ maxWidth: '160px' }} value={subjFilter} onChange={e => setSubjFilter(e.target.value)}>
                ...
                {/* Add/Edit form */}
                {showQForm && (
                  <DashCard glowColor="blue" style={{ borderColor: 'rgba(43,175,252,0.3)', padding: '20px' }}>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div>
                          ...
                          <select className="input-field text-sm" value={qForm.exam_type}
                            onChange={e => { updQ('exam_type', e.target.value); updQ('subject_id', '') }}>
                            {ALL_EXAMS.map(et => <option key={et}>{et}</option>)}
                          </select>
                        </div>
                        <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="flex items-end pb-1">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={qForm.has_math} onChange={e => updQ('has_math', e.target.checked)} className="w-4 h-4 rounded" />
                            <span className="text-sm" style={{ color: 'rgba(244,249,253,0.6)' }}>Contains math ($formula$)</span>
                          </label>
                        </div>
                      </div>
                      ...
                      <div>
                        <label className="text-xs font-mono-display block mb-1.5" style={{ color: 'rgba(244,249,253,0.5)' }}>
                          QUESTION TEXT {qForm.has_math && <span style={{ color: '#2baffc' }}>· use $x^2$ for math</span>}
                        </label>
                        <textarea className="input-field resize-none text-sm" rows={4}
                          placeholder="Enter the question..." value={qForm.question_text}
                          onChange={e => updQ('question_text', e.target.value)} />
                        {qForm.has_math && qForm.question_text && (
                          <div className="mt-1.5 p-2 rounded text-sm" style={{ background: '#0a0a0b', border: '1px solid #1e1e24' }}>
                            <span className="text-xs font-mono-display mr-1" style={{ color: 'rgba(244,249,253,0.4)' }}>Preview:</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {(['A', 'B', 'C', 'D'] as const).map(opt => {
                          const key = `option_${opt.toLowerCase()}` as keyof QForm
                          return (
                            <div key={opt}>
                      ...
                      <div>
                        ...
                        <textarea className="input-field resize-none text-sm" rows={3}
                          placeholder="Step-by-step solution..." value={qForm.explanation}
                          onChange={e => updQ('explanation', e.target.value)} />
                      </div>
                ...
                <DashCard glowColor="blue" style={{ padding: 0 }}>
                  ...
                  {filteredQ.length === 0 ? (
                    ...
                    <div className="divide-y" style={{ borderColor: '#1e1e24' }}>
                      {filteredQ.map(q => (
                        <div key={q.id} className="flex items-start gap-3 p-4 transition-all hover:bg-[#0a0a0b]">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className="text-xs font-mono-display px-1.5 py-0.5 rounded"
                                style={{ background: (EXAM_COLORS[q.exam_type] || '#2baffc') + '15', color: EXAM_COLORS[q.exam_type] || '#2baffc' }}>
                                {q.exam_type}
                              </span>
                              ...
                              <span className="text-xs px-1.5 py-0.5 rounded"
                                style={{ background: q.difficulty === 'easy' ? 'rgba(85,195,96,0.1)' : q.difficulty === 'hard' ? 'rgba(255,107,107,0.1)' : 'rgba(43,175,252,0.1)', color: q.difficulty === 'easy' ? '#55c360' : q.difficulty === 'hard' ? '#ff8080' : '#2baffc' }}>
                                {q.difficulty}
                              </span>
                          ...
                          <div className="flex gap-1 flex-shrink-0">
                            ...
                            <button onClick={() => deleteQ(q.id)} className="p-2 rounded-lg transition-all" style={{ color: 'rgba(244,249,253,0.4)' }}
            ...
            {tab === 'pdf' && (
              <div className="space-y-5 animate-in">
                <DashCard glowColor="blue" style={{ padding: '20px' }}>
                  <h2 className="font-mono-display font-bold text-sm mb-1" style={{ color: '#f4f9fd' }}>PDF / Text Import</h2>
                  <p className="text-xs mb-4" style={{ color: 'rgba(244,249,253,0.5)' }}>Paste question text or upload .txt — AI extracts and categorises automatically.</p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                    ...
                    <div>
                      <label className="text-xs font-mono-display block mb-1.5" style={{ color: 'rgba(244,249,253,0.5)' }}>SUBJECT HINT</label>
                    ...
                    <div className="flex items-end">
                      <label className="btn-ghost flex items-center gap-2 py-2.5 px-4 text-sm cursor-pointer w-full justify-center">
                        <Upload size={14} /> Upload .txt
                        <input ref={fileRef} type="file" accept=".txt" className="hidden" onChange={handleFileUpload} />
                      </label>
                    </div>
                  </div>
                  <textarea className="input-field resize-none text-sm mb-4" rows={10}
                    placeholder="Paste questions here..." value={pdfText} onChange={e => setPdfText(e.target.value)} />
                  {pdfError && <div className="mb-3 px-3 py-2 rounded text-xs" style={{ background: 'rgba(255,107,107,0.1)', color: '#ff8080', border: '1px solid rgba(255,107,107,0.2)' }}>{pdfError}</div>}
                  <button onClick={extractQuestions} disabled={extracting || !pdfText.trim()} className="btn-primary flex items-center gap-2">
                    {extracting ? <><Loader2 size={14} className="animate-spin" /> Extracting...</> : <><FileText size={14} /> Extract with AI</>}
                  </button>
                </DashCard>

                {extractedQs.length > 0 && (
                  <DashCard glowColor="green" style={{ borderColor: 'rgba(85,195,96,0.3)', padding: '20px' }}>
                    <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                      <div>
                        <h3 className="font-mono-display font-bold text-sm" style={{ color: '#55c360' }}>✓ {extractedQs.length} Questions Extracted</h3>
                        <p className="text-xs mt-0.5" style={{ color: 'rgba(244,249,253,0.4)' }}>Click to deselect any you don&apos;t want</p>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setSelectedExtracted(new Set(extractedQs.map((_, i) => i)))} className="btn-ghost text-xs py-1.5 px-3">All</button>
                    ...
                    <div className="space-y-3 max-h-[500px] overflow-auto">
                      {extractedQs.map((q, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => { const n = new Set(selectedExtracted); sel ? n.delete(i) : n.add(i); setSelectedExtracted(n) }}
                            className="p-4 rounded-xl cursor-pointer transition-all"
                            style={{ background: sel ? 'rgba(85,195,96,0.05)' : '#0a0a0b', border: `1px solid ${sel ? 'rgba(85,195,96,0.3)' : '#1e1e24'}` }}>
                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                              <div className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0"
                                style={{ background: sel ? '#55c360' : '#1e1e24', border: `1px solid ${sel ? '#55c360' : '#2a2a32'}` }}>
                                {sel && <Check size={9} style={{ color: '#010101' }} />}
                              </div>
                              <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'rgba(43,175,252,0.1)', color: '#2baffc' }}>{q.subject}</span>
                              <span className="text-xs" style={{ color: 'rgba(244,249,253,0.4)' }}>{q.topic}</span>
                              <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: q.difficulty === 'easy' ? 'rgba(85,195,96,0.1)' : q.difficulty === 'hard' ? 'rgba(255,107,107,0.1)' : 'rgba(43,175,252,0.1)', color: q.difficulty === 'easy' ? '#55c360' : q.difficulty === 'hard' ? '#ff8080' : '#2baffc' }}>{q.difficulty}</span>
                            </div>
                            <p className="text-sm" style={{ color: '#f4f9fd' }}><MathText text={q.question_text} /></p>
                          </div>
                        )
                      })}
                    </div>
            ...
            {tab === 'exams' && (
              <div className="grid sm:grid-cols-2 gap-4 animate-in">
                {mockExams.map(e => (
                  <DashCard key={e.id} glowColor={e.is_active ? 'green' : 'blue'}
                    style={{ borderColor: e.is_active ? 'rgba(85,195,96,0.2)' : '#1e1e24', padding: '20px' }}>
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        <span className="text-xs font-mono-display px-2 py-0.5 rounded mb-2 inline-block"
                          style={{ background: (EXAM_COLORS[e.exam_type] || '#2baffc') + '15', color: EXAM_COLORS[e.exam_type] || '#2baffc' }}>
                          {e.exam_type}
                        </span>
            ...
            {/* ── STUDENTS ── */}
            {tab === 'students' && (
              <DashCard glowColor="blue" style={{ padding: 0 }} className="animate-in">
                <div className="p-4" style={{ borderBottom: '1px solid #1e1e24' }}>
                ...
                {profiles.length === 0 ? (
                  ...
                  <div className="divide-y overflow-x-auto" style={{ borderColor: '#1e1e24' }}>
                    {profiles.map(p => (
                      <div key={p.id} className="flex items-center gap-3 px-4 py-3 min-w-0 transition-all hover:bg-[#0a0a0b]">
                        ...
                        <span className="text-xs flex-shrink-0 hidden sm:block" style={{ color: 'rgba(244,249,253,0.3)' }}>
