...
import { Shield, Eye, EyeOff, Users, BookOpen, BarChart3, Plus, Trash2, Edit3, X, Check } from 'lucide-react'
...
interface Question {
  ...
  year: number | null; is_active: boolean
...
interface QuestionForm {
  ...
  option_a: string; option_b: string; option_c: string; option_d: string
  correct_option: string; explanation: string; difficulty: string; topic: string; year: string
}

const EMPTY_Q: QuestionForm = {
  exam_type: 'JEE', subject_id: '', question_text: '',
  option_a: '', option_b: '', option_c: '', option_d: '',
  correct_option: 'A', explanation: '', difficulty: 'medium', topic: '', year: '',
}
...
function AdminPanel() {
  ...
  async function loadAll() {
    const [{ data: q }, { data: p }, { data: s }, { data: m }] = await Promise.all([
      supabase.from('questions').select('*, subjects(name)').order('created_at', { ascending: false }).limit(100),
  ...
  function updQ(k: keyof QuestionForm, v: string) { setQForm(f => ({ ...f, [k]: v })) }
  ...
  function startEdit(q: Question) {
    setQForm({ ...q, year: q.year?.toString() || '', subject_id: q.subject_id || '' })
    setEditId(q.id)
    setShowQForm(true)
    window.scrollTo(0, 0)
  }
  ...
  const filteredQ = qFilter
    ? questions.filter(q => q.question_text.toLowerCase().includes(qFilter.toLowerCase()) || q.topic?.toLowerCase().includes(qFilter.toLowerCase()))
    : questions
        ...
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={{ background: tab === t.id ? '#2baffc' : 'transparent', color: tab === t.id ? '#010101' : 'rgba(244,249,253,0.5)' }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                          ...
                          <select className="input-field text-sm" value={qForm.exam_type} onChange={e => updQ('exam_type', e.target.value)}>
                        ...
                        <div>
                          ...
                          <select className="input-field text-sm" value={qForm.subject_id} onChange={e => updQ('subject_id', e.target.value)}>
                            ...
                            {subjects.filter(s => s.exam_type === qForm.exam_type).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                          </select>
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <textarea className="input-field resize-none text-sm" rows={4} placeholder="Enter the question..." value={qForm.question_text} onChange={e => updQ('question_text', e.target.value)} />
                      </div>
                          ...
                          return (
                            <div key={opt}>
                              ...
                              <input className="input-field text-sm" placeholder={`Option ${opt}...`}
                                value={qForm[key]} onChange={e => updQ(key, e.target.value)} />
                            </div>
                          )
                        })}
                      </div>
                      ...
                      {filteredQ.map(q => (
                        <div key={q.id} className="flex items-start gap-4 p-4 transition-all hover:bg-[#0a0a0b]">
                          ...
                          <div className="flex gap-1 flex-shrink-0">
                            ...
                            <button onClick={() => deleteQuestion(q.id)} className="p-2 rounded-lg transition-all"
                              style={{ color: 'rgba(244,249,253,0.4)' }}
            ...
            {tab === 'students' && (
              <div className="card" style={{ borderColor: '#1e1e24', padding: 0 }}>
                ...
                {profiles.length === 0 ? (
                  ...
                  <div className="divide-y" style={{ borderColor: '#1e1e24' }}>
                    {profiles.map(p => (
                      <div key={p.id} className="flex items-center gap-4 px-4 py-3 transition-all hover:bg-[#0a0a0b]">
