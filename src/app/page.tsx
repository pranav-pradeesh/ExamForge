...
export default function HomePage() {
  ...
  function navAfterAnim(href: string) { return () => { setTimeout(() => router.push(href), 1800) } }
    ...
    <div className="min-h-screen" style={{ background: '#010101' }}>
      ...
      <section className="relative pt-28 sm:pt-36 pb-16 sm:pb-24 px-4 sm:px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border mb-6 text-xs font-mono-display ${mounted ? 'animate-in' : 'opacity-0'}`}
            style={{ background: 'rgba(43,175,252,0.08)', borderColor: 'rgba(43,175,252,0.3)', color: '#2baffc' }}>
            <Zap size={11} />
            <span className="hidden xs:inline">JEE · VITEEE · KEAM · CUSAT · NEET</span>
            <span className="xs:hidden">5 Exams Covered</span>
          </div>

          <h1 className={`font-mono-display font-bold leading-none mb-5 ${mounted ? 'animate-in stagger-1' : 'opacity-0'}`}
            style={{ fontSize: 'clamp(2.4rem, 9vw, 5.5rem)', color: '#f4f9fd', letterSpacing: '-0.03em' }}>
            Forge Your<br />
            <span style={{ color: '#2baffc' }}>Exam</span> Rank.
          </h1>

          <p className={`text-base sm:text-lg mb-8 max-w-xl mx-auto leading-relaxed px-2 ${mounted ? 'animate-in stagger-2' : 'opacity-0'}`}
          ...
          <div className={`flex flex-col xs:flex-row items-center justify-center gap-3 ${mounted ? 'animate-in stagger-3' : 'opacity-0'}`}>
            <ParticleButton variant="primary" className="w-full xs:w-auto px-8 py-3.5 text-sm" successDuration={1800} onSuccess={navAfterAnim('/signup')}>
              Start Free <ArrowRight size={15} />
            </ParticleButton>
            <ParticleButton variant="ghost" className="w-full xs:w-auto px-8 py-3.5 text-sm" successDuration={1800} onSuccess={navAfterAnim('/login')}>
              Sign In
            </ParticleButton>
          </div>

          {/* Stats */}
          <div className={`mt-12 grid grid-cols-3 gap-px rounded-xl overflow-hidden ${mounted ? 'animate-in stagger-4' : 'opacity-0'}`}
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map(f => {
              const colorKey = f.color === '#55c360' ? 'green' : f.color === '#f59e0b' ? 'amber' : 'blue'
              return (
                <GlowCard key={f.title} glowColor={colorKey as 'blue'|'green'|'amber'}
                  style={{ background: '#111114', border: '1px solid #1e1e24', padding: '20px' }}>
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-4" style={{ background: f.color + '15', color: f.color }}>{f.icon}</div>
              ...
              <thead>
                <tr style={{ background: '#0a0a0b', borderBottom: '1px solid #1e1e24' }}>
                  {['Exam','Mode','Duration','Questions','Marks','Marking'].map(h => (
                    <th key={h} className="px-3 sm:px-4 py-3 text-left font-mono-display text-xs whitespace-nowrap" style={{ color: 'rgba(244,249,253,0.5)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto text-center">
          <div className="relative rounded-2xl p-8 sm:p-12" style={{ background: 'linear-gradient(135deg, rgba(43,175,252,0.1) 0%, rgba(85,195,96,0.05) 100%)', border: '1px solid rgba(43,175,252,0.2)' }}>
