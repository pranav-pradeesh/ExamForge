  ...
  useEffect(() => {
    async function load() {
      ...
      if (sessions && sessions.length > 0) {
        ...
        const avgScore = sessions.reduce((a, s) => a + s.percentage, 0) / total
        const bestScore = Math.max(...sessions.map(s => s.percentage))
        const physAvg = sessions.reduce((a, s) => a + (s.physics_score || 0), 0) / total
        const chemAvg = sessions.reduce((a, s) => a + (s.chemistry_score || 0), 0) / total
        const mathsAvg = sessions.reduce((a, s) => a + (s.maths_score || 0), 0) / total

        // Extract weak topics from AI analysis
        const weakTopics: string[] = []
        sessions.forEach(s => {
          if (s.ai_weak_topics) weakTopics.push(...(s.ai_weak_topics || []))
        })
        const uniqueWeak = [...new Set(weakTopics)].slice(0, 6)

        setStats({
          totalTests: total,
          avgScore: Math.round(avgScore * 10) / 10,
          bestScore: Math.round(bestScore * 10) / 10,
          physicsAvg: Math.round(physAvg),
          chemAvg: Math.round(chemAvg),
          mathsAvg: Math.round(mathsAvg),
        ...
        setStats({
          ...
          physicsAvg: 0, chemAvg: 0, mathsAvg: 0,
          recentSessions: [], weakTopics: [],
        })
      }
      setLoading(false)
    }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm" style={{ color: 'rgba(244,249,253,0.4)' }}>Loading your dashboard...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid lg:grid-cols-5 gap-5 animate-in stagger-2">
        {/* Nebula */}
        <div className="lg:col-span-2 card relative overflow-hidden" style={{ borderColor: '#1e1e24' }}>
          <div className="absolute inset-0 pointer-events-none" style={{ opacity: 0.25 }}>
            <div className="absolute inset-0 rounded-xl"
              style={{
                background: getNebulaGradient(physPct, chemPct, mathPct),
                filter: 'blur(32px)',
          ...
          <div className="relative">
            <div className="text-xs font-mono-display mb-4 flex items-center gap-2" style={{ color: 'rgba(244,249,253,0.5)' }}>
              <Zap size={12} style={{ color: '#2baffc' }} />
              PROGRESS NEBULA
            </div>
            ...
            {!hasData && (
              <div className="mt-6 text-center">
                <p className="text-xs" style={{ color: 'rgba(244,249,253,0.35)' }}>Complete a test to see your nebula</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Radar */}
        <div className="lg:col-span-3 card" style={{ borderColor: '#1e1e24' }}>
          ...
          {hasData ? (
            <ResponsiveContainer width="100%" height={200}>
              <RadarChart data={radarData}>
                ...
                <Tooltip
                  contentStyle={{ background: '#111114', border: '1px solid #1e1e24', borderRadius: 8 }}
                  labelStyle={{ color: '#f4f9fd', fontSize: 12 }}
                  itemStyle={{ color: '#2baffc', fontSize: 12 }}
                />
              </RadarChart>
            </ResponsiveContainer>
          ) : (
      ...
      <div className="grid lg:grid-cols-3 gap-5 animate-in stagger-3">
        {/* Quick start */}
        <div className="card space-y-3" style={{ borderColor: '#1e1e24' }}>
          <div className="text-xs font-mono-display mb-2" style={{ color: 'rgba(244,249,253,0.5)' }}>QUICK START</div>
          {[
            { label: 'JEE Full Mock', sub: '90 questions · 3h', color: '#2baffc', href: '/dashboard/exam?type=JEE&mode=full' },
            { label: 'VIT Full Mock', sub: '125 questions · 2.5h', color: '#55c360', href: '/dashboard/exam?type=VIT&mode=full' },
            { label: 'Chapter Practice', sub: 'Pick topic & difficulty', color: '#f59e0b', href: '/dashboard/exam?mode=chapter' },
          ].map(q => (
            <Link key={q.label} href={q.href}
              className="flex items-center gap-3 p-3 rounded-xl transition-all group"
              style={{ background: '#0a0a0b', border: '1px solid #1e1e24' }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = q.color + '40')}
        ...
        {/* Recent tests */}
        <div className="lg:col-span-2 card" style={{ borderColor: '#1e1e24' }}>
          ...
          {hasData ? (
                ...
                return (
                  <Link key={s.id} href={`/dashboard/results/${s.id}`}
                    ...
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center font-mono-display font-bold text-sm flex-shrink-0"
                      style={{ background: scoreColor(pct) + '15', color: scoreColor(pct) }}>
                      {pct}%
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs flex items-center gap-3 mt-0.5">
                        ...
                        <span className="flex items-center gap-1" style={{ color: 'rgba(244,249,253,0.4)' }}>
                          <Clock size={10} />
                          {formatTime(s.time_taken_seconds || 0)}
                        </span>
                      </div>
                    </div>
            ...
            <div className="h-40 flex items-center justify-center">
              <div className="text-center">
                ...
                <Link href="/dashboard/exam" className="btn-primary text-xs py-2 px-4 flex items-center gap-2 mx-auto w-fit">
                  <Play size={12} /> Take First Test
                </Link>
              </div>
            </div>
          )}
      ...
      {stats && stats.weakTopics.length > 0 && (
        <div className="card animate-in stagger-4" style={{ borderColor: '#1e1e24' }}>
          <div className="text-xs font-mono-display mb-4" style={{ color: 'rgba(244,249,253,0.5)' }}>
            AI-IDENTIFIED WEAK ZONES
          </div>
          <div className="flex flex-wrap gap-2">
            {stats.weakTopics.map(t => (
              <span key={t} className="text-xs px-3 py-1.5 rounded-full font-medium"
                style={{ background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.3)', color: '#ff8080' }}>
                {t}
              </span>
            ))}
          </div>
          <p className="mt-3 text-xs" style={{ color: 'rgba(244,249,253,0.4)' }}>
            Based on your recent test performance. Focus on these first.
          </p>
        </div>
      )}
    </div>
